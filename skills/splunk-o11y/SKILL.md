...
Splunk Observability CloudのREST APIには、条件を指定してトレースを検索するエンドポイントが存在しない（`/v2/apm/traces`, `/v2/apm/trace/search` 等は404）。`get_trace.py` はトレースIDが既知の場合にのみ使用できる。

**対処**: トレースIDの特定が必要な場合は、ユーザーにSplunk APM UIでトレースを検索してもらい、トレースIDを共有してもらう。

### SignalFlowのAPMメトリクスはオペレーション単位でフィルタできない

`get_service_metrics.py` が使用するSignalFlow APMメトリクス（`service.request.count` 等）は `sf_service` でのフィルタは可能だが、`sf_operation` でのフィルタは `numInputTimeSeries: 0` となり機能しない。

**対処**: サービス全体のエラー率・スループットのみ取得可能。オペレーション別の分析が必要な場合はユーザーにAPM UIでの確認を依頼する。

### サービスメトリクスからトレースIDへの導線がない

エラー率やスループットの異常は検出できるが、APIだけでは「どのトレースが問題か」を特定する手段がない。サービスレベルの概況把握までがこのスキルの対応範囲となる。
...
4. **問題のトレースを特定**: ユーザーにAPM UIでエラートレースを検索してもらい、トレースIDを取得する
5. **トレース詳細を取得**: `get_trace.py` でスパン一覧・属性を確認
